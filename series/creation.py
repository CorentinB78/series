...
def log_series(N, x0=1.):
    """
    Taylor series of f(x) = ln(x0 + x), for x close to zero, up to order N (included).
    """
    s = np.empty(N+1, dtype=complex)
    s[0] = np.log(x0)
    s[1:] = -1. / np.arange(1, N+1, dtype=float)
    return rescale_series(s, -1. / x0)

def exp_series(N, x0=0.):
    """
...
def cos_series(N, x0=0.):
    """
    Taylor series of f(x) = cos(x0 + x), for x close to zero, up to order N (included).
    Not tested !
    """
    s_plus = rescale_series(exp_series(N, x0=1.j*x0), 1.j)
    s_minus = rescale_series(exp_series(N, x0=-1.j*x0), -1.j)
    return 0.5 * (s_plus + s_minus)

def sin_series(N, x0=0.):
    """
