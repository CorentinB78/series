...
def compose_series(series1, series2, axis1=-1, axis2=-1):
    ...
    if (series2_[0] != 0).any():
        raise ValueError(f'constant coefficient of series2 must be zero, but is {series2_[0]}')
...
def reverse_series(series, axis=0):
    ...
    output[1] = 1. / series_[1]

    der_series = np.append(series_[1:] * np.arange(1, len(series_)), [0.])
